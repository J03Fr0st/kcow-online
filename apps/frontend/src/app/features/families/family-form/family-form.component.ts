...
import { FamilyService, Family, Guardian, CreateGuardianRequest } from '@core/services/family.service';
import { NotificationService } from '@core/services/notification.service';
import { GuardianFormComponent, GuardianFormData } from '@shared/components/guardian-form/guardian-form.component';
...
export class FamilyFormComponent implements OnInit {
        ...
        this.familyService.getFamilyById(id).pipe(
            ...
            error: (error) => {
                this.notificationService.error(
                    error.detail || 'Failed to load family',
                    'Error'
                );
    ...
    protected onSubmit(): void {
        ...
        if (this.isEditMode()) {
            ...
            this.familyService.updateFamily(this.familyId()!, updateRequest).subscribe({
                next: () => {
                ...
                error: (error) => {
                    this.notificationService.error(
                        error.detail || 'Failed to update family',
                        'Error'
                    );
            ...
            if (guardiansList.length === 0) {
                this.notificationService.warning('Please add at least one guardian to create a family', 'Validation');
                this.isSubmitting.set(false);
                return;
            }
            ...
            this.familyService.createFamily(request).subscribe({
                next: (family: Family) => {
                    ...
                    this.familyService.addGuardian(family.id, guardianRequest).subscribe({
                        next: () => {
                            this.notificationService.success('Family and guardian created successfully', 'Success');
                            this.router.navigate(['/families']);
                        },
                        error: (error) => {
                            this.notificationService.error(
                ...
                error: (error) => {
                    this.notificationService.error(
                        error.detail || 'Failed to create family',
                        'Error'
                    );
                    this.isSubmitting.set(false);
                },
    ...
    protected getErrorMessage(controlName: string): string {
        ...
        const label = controlName.charAt(0).toUpperCase() + controlName.slice(1).replace(/([A-Z])/g, ' $1');
