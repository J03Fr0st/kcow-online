...
describe('FamilyListComponent', () => {
  ...
  const mockFamilies = [
    {
      ...
      guardians: [{ id: 1, firstName: 'John', lastName: 'Smith', isPrimaryContact: true }],
    },
    {
      id: 2,
  ...
  it('should show deactivate button for active families only', () => {
    ...
    const smithFamilyRow = rows.find((row) => {
      ...
      return nameCell?.nativeElement.textContent.includes('Smith Family');
    });
    expect(smithFamilyRow).toBeTruthy();

    // Smith Family is active - should have deactivate button
    const deactivateButtonA = smithFamilyRow?.query(
      By.css('button[aria-label="Deactivate family"]'),
    );
    expect(deactivateButtonA).toBeTruthy();

    // Smith Family should NOT have reactivate button
    const reactivateButtonA = smithFamilyRow?.query(
      By.css('button[aria-label="Reactivate family"]'),
    );
    ...
    const johnsonFamilyRow = rows.find((row) => {
      const nameCell = row.query(By.css('td'));
      return nameCell?.nativeElement.textContent.includes('Johnson Family');
    });
    expect(johnsonFamilyRow).toBeTruthy();

    // Johnson Family is inactive - should have reactivate button
    const reactivateButtonB = johnsonFamilyRow?.query(
      By.css('button[aria-label="Reactivate family"]'),
    );
  ...
  it('should open confirmation dialog when deactivate button is clicked', async () => {
    ...
    expect(mockModalService.confirm).toHaveBeenCalledWith({
      ...
      message:
        'Are you sure you want to deactivate "Smith Family"? It will no longer be available for new student assignments.',
  ...
  it('should show error notification when deactivate fails', async () => {
    mockFamilyService.deactivateFamily.mockReturnValue(
