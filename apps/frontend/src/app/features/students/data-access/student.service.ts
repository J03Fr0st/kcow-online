...
import { catchError, finalize, map, type Observable, of, tap, throwError } from 'rxjs';
import { environment } from '@environments/environment';

export interface Student {
  // Optional family projection; not fields in the persisted legacy student schema.
...
export class StudentService {
  ...
  getStudents(params: GetStudentsParams = {}): Observable<PaginatedStudentsResponse> {
    ...
    return this.http
      .get<BackendPagedResponse>(this.apiUrl, {
  ...
  searchStudents(query: string, limit: number = 10): Observable<StudentSearchResult[]> {
    ...
    const httpParams = new HttpParams().set('q', query.trim()).set('limit', limit.toString());

    return this.http
      .get<StudentSearchResult[]>(`${this.apiUrl}/search`, {
        withCredentials: true,
        params: httpParams,
      })
      .pipe(
