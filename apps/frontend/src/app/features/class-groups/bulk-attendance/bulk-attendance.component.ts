...
export class BulkAttendanceComponent implements OnInit {
  ...
  private loadClassGroupInfo(): void {
    ...
    this.http
      .get<ClassGroup>(`${environment.apiUrl}/class-groups/${this.classGroupId()}`)
      .pipe(
  ...
  private loadStudents(): void {
    ...
    this.studentService
      .getStudents({ classGroupId: this.classGroupId() })
      ...
      .subscribe({
        next: (response) => {
          ...
          const entries: StudentAttendanceEntry[] = students.map((student) => ({
            ...
            studentName:
              `${student.firstName || ''} ${student.lastName || ''}`.trim() || student.reference,
            grade: student.grade,
  ...
  private loadExistingAttendance(): void {
    ...
    this.http
      .get<Attendance[]>(
        `${environment.apiUrl}/attendance?classGroupId=${this.classGroupId()}&fromDate=${date}&toDate=${date}`,
      )
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        catchError((error: HttpErrorResponse) => {
  ...
  protected updateStudentStatus(studentId: number, status: AttendanceStatus): void {
    this.students.update((entries) =>
      entries.map((entry) => (entry.studentId === studentId ? { ...entry, status } : entry)),
    );
  }
  ...
  protected updateStudentNotes(studentId: number, notes: string): void {
    this.students.update((entries) =>
      entries.map((entry) => (entry.studentId === studentId ? { ...entry, notes } : entry)),
    );
  }
  ...
  protected saveAll(): void {
    ...
    this.http
      .post<BatchAttendanceResponse>(
        `${environment.apiUrl}/class-groups/${this.classGroupId()}/attendance`,
        request,
      )
      .pipe(
        takeUntilDestroyed(this.destroyRef),
