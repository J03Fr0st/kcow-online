...
export class FamilyService {
  ...
  getGuardiansByFamily(familyId: number): Observable<Guardian[]> {
    return this.http
      .get<Guardian[]>(`${this.apiUrl}/${familyId}/guardians`, { withCredentials: true })
      .pipe(
        catchError((error: HttpErrorResponse) => {
          return this.handleError(error);
        }),
  ...
  addGuardian(familyId: number, guardian: CreateGuardianRequest): Observable<Guardian> {
    return this.http
      .post<Guardian>(`${this.apiUrl}/${familyId}/guardians`, guardian, { withCredentials: true })
      .pipe(
        tap((newGuardian) => {
  ...
  updateGuardian(
    ...
    return this.http
      .put<Guardian>(`${this.apiUrl}/${familyId}/guardians/${guardianId}`, guardian, {
        withCredentials: true,
      })
      .pipe(
        tap((updatedGuardian) => {
          ...
          this.familiesSignal.update((families) =>
            families.map((f) => {
              if (f.id === familyId && f.guardians) {
                return {
                  ...f,
                  guardians: f.guardians.map((g) => (g.id === guardianId ? updatedGuardian : g)),
                };
              }
              return f;
            }),
  ...
  deleteGuardian(familyId: number, guardianId: number): Observable<void> {
    return this.http
      .delete<void>(`${this.apiUrl}/${familyId}/guardians/${guardianId}`, { withCredentials: true })
      .pipe(
        tap(() => {
  ...
  getStudentsByFamily(familyId: number): Observable<StudentSummary[]> {
    return this.http
      .get<StudentSummary[]>(`${this.apiUrl}/${familyId}/students`, { withCredentials: true })
      .pipe(
        catchError((error: HttpErrorResponse) => {
          return this.handleError(error);
        }),
  ...
  previewMerge(request: MergeFamiliesRequest): Observable<FamilyMergePreview> {
    return this.http
      .post<FamilyMergePreview>(`${this.apiUrl}/merge/preview`, request, { withCredentials: true })
      .pipe(
        catchError((error: HttpErrorResponse) => {
          return this.handleError(error);
        }),
  ...
  mergeFamilies(request: MergeFamiliesRequest): Observable<MergeFamiliesResult> {
    return this.http
      .post<MergeFamiliesResult>(`${this.apiUrl}/merge`, request, { withCredentials: true })
      .pipe(
        tap((_result) => {
