...
public class RoleRepository : IRoleRepository
{
    ...
    public async Task<bool> ExistsByNameAsync(string name, CancellationToken cancellationToken = default)
    {
        ...
        var count = await connection.QuerySingleAsync<int>(new CommandDefinition(sql, new { Name = name }, cancellationToken: cancellationToken));
        return count > 0;
    }
    ...
    public async Task<int> CreateAsync(Role role, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            RETURNING Id";
        return await connection.QuerySingleAsync<int>(new CommandDefinition(sql, role, cancellationToken: cancellationToken));
    }
    ...
    public async Task<bool> UpdateAsync(Role role, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            SET Name = @Name
            WHERE Id = @Id";
        var rowsAffected = await connection.ExecuteAsync(new CommandDefinition(sql, role, cancellationToken: cancellationToken));
