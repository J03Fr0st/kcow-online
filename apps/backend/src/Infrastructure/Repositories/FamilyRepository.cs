...
public class FamilyRepository : IFamilyRepository
{
    ...
    public async Task<IEnumerable<Family>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            FROM families";
        return await connection.QueryAsync<Family>(new CommandDefinition(sql, cancellationToken: cancellationToken));
    }
    ...
    public async Task<Family?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            FROM families
            WHERE id = @Id";
        return await connection.QueryFirstOrDefaultAsync<Family>(new CommandDefinition(sql, new { Id = id }, cancellationToken: cancellationToken));
    }
    ...
    public async Task<int> CreateAsync(Family family, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            RETURNING id";
        return await connection.QuerySingleAsync<int>(new CommandDefinition(sql, family, cancellationToken: cancellationToken));
    }
    ...
    public async Task<bool> UpdateAsync(Family family, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            ...
            WHERE id = @Id";
        var rowsAffected = await connection.ExecuteAsync(new CommandDefinition(sql, family, cancellationToken: cancellationToken));
